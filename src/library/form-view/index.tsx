...
const FormInternal = (props: {
  ...
  if (props.onFormEvent) {
    store.getState().onFormEvent = props.onFormEvent;
  }

  const { formRef, activePage } = store(
    useShallow(state => ({
  ...
  const nextPage = () => {
    ...
    if (targetSchema.type === 'object') {
      ...
      thisSchema.items = deepCopy(targetSchema.items);
      thisSchema.items.required = targetSchema.items.inputRequired
        ? [targetSchema.items.name]
        : [];
    }
        ...
        {schema?.pages?.map((page, idx) => {
          return (
            <button
              ...
              onClick={e => setPage(idx)}
              className={classNames(
                idx === activePage
                  ? tabButtonActiveClass + ' text-gray-600'
      ...
      <div className="flex justify-between text-sm my-2">
        <button
          className={classNames(
            ...
            ' hover:bg-gray-100 disabled:hover:bg-white py-1 px-4 rounded-lg shadow disabled:text-gray-200'
          )}
          onClick={prevPage}
        >
          Back
        ...
        <button
          className="hover:bg-gray-100 py-1 px-4 rounded-lg shadow"
          onClick={nextPage}
        >
