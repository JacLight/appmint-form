...
const FormStoreContext = createContext<UseBoundStore<
  StoreApi<FormStoreProps>
> | null>(null);

// Create a factory function for form stores
const createFormStore = () =>
  create<FormStoreProps>((set, get) => ({
    storeId: '',
    accessMode: '1',
    activePage: 0,
    error: {},
    ...
    updateBulkError: (validation: {
      valid;
      errors;
      message;
      validationResults: { path; error; message }[];
    }) =>
      set(state => {
        if (validation?.valid) {
          return {
            error: {},
            timestamp: { ...state.timestamp, ['root']: Date.now() },
          };
        }
    ...
    updateError: (dataPath: string, message?: string) =>
      set(state => {
        ...
        return {
          error: error,
          timestamp: { ...state.timestamp, [dataPath]: Date.now() },
        };
      }),
    clearError: () =>
      set(state => ({
    ...
    getError: (dataPath: string = '', startsWith = false) => {
      if (!get().error) return;
      if (startsWith) {
        const keys = Object.keys(get().error).filter(key =>
