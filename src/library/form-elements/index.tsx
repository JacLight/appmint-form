...
import {
  applyFormTransform,
  applyFunction,
} from '../form-view/form-transforms';
...
export const FormElementRender = (props: {
  storeId;
  theme?: any;
  mode: string;
  name: string;
  ...
  useEffect(() => {
    if (schema?.fn && dataPathTimestamp) {
      const fn = schema.fn.startsWith('return')
        ? schema.fn
        : `return  ${schema.fn} `;
      const fnResult = applyFunction(
        fn,
        value,
        getItemValue(parentPath),
        getItemValue()
      );
  ...
  if (schema && typeof schema !== 'object') {
    console.error(
      'FormElementRender: schema must be an object',
      name,
      path,
      arrayIndex,
      schema
  ...
  const handleBlur = async (newValue, isFile?) => {
    ...
    updateError(dataPath, null);
    const validationResult = validateFormValue(
      dataPath,
      newValue,
      schema,
      getItemValue()
      ...
      if (schema.unique && newValue) {
        try {
          if (store.getState().onFormEvent) {
            const isUnique = await store.getState().onFormEvent('isUnique', {
              datatype,
              field: schema.name,
              value: newValue,
            });
    ...
    if (schema.fetchData?.datatype) {
      ...
      if (store.getState().onFormEvent) {
        store
          .getState()
          .onFormEvent('getDataById', {
            datatype: schema.fetchData?.datatype,
            id: newValue,
          })
          .then(res => {
  ...
  if (props.mode === 'view' && (ruleSchema.hidden || ruleActions.hide))
    return null;
  ...
  let controlType = ruleSchema.fieldType
    ? ruleSchema.fieldType
    : getControlType(ruleSchema);
  ...
  ruleSchema.placeholder = ruleSchema?.placeholder
    ? ruleSchema.placeholder
    : toSentenceCase(ruleSchema?.title || ruleSchema?.name || name || '');
  let render = (
    ...
    >
      <Element
        ...
        className={twMerge(
          'placeholder:text-xs',
          inputTheme?.className,
          controlTheme?.className,
          uiInputStyle?.classes?.join(' '),
          uiControlStyle?.classes?.join(' ')
        )}
      />
    </ElementWrapperControl>
  );
  ...
  if (ruleSchema.collapsible) {
    render = (
      <FormCollapsible
        defaultState={ruleSchema.collapsible}
        theme={theme}
        title={caption}
        icon={ruleSchema.icon}
        ui={ruleSchema['x-ui']}
      >
        {render}
      </FormCollapsible>
  ...
  if (ruleSchema.popup) {
    render = (
      <FormPopup
        title={ruleSchema.title || ruleSchema.name}
        icon={ruleSchema.icon}
        ui={ruleSchema['x-ui']}
        theme={theme}
        popupStyle={ruleSchema.popup}
      >
        {render}
      </FormPopup>
    );
  }
  store.getState().updateRenderedElement(path, { element: render });
  return render;
};
